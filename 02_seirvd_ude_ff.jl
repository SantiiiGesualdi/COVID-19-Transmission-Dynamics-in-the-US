...
function load_data(state_name, data_path, vac_path, total_pop, t_vac_default)
    ...
    return t_obs, C_obs, D_obs, t_vac, V_acc_obs, t_init_vac, tspan
end
...
function SEIRVD_Dynamics(u, t, t_past, h, p_const, p_time, p_time_past, eps, t_init_vac)
    ...
    # Soft shield: pushes any negative to ~0 without breaking the derivative
    ...
    # Application of past infection rates (NOT TODAY)
...
function f_neural_dde(u, h, p_global, t, t0_val, T_val, t_init_vac, st)
    ...
    # Calculate past time ONLY ONCE
    τ_safe = 6.0 * logistic(p_global.ode.τ * 0.1) + 3.0
    t_past = t - τ_safe
...
function predict(θ_actual, t_max, t_obs_data, prob_base, u_base, t0_val)
    τ_safe_val = Zygote.ignore_derivatives(() -> 6.0 * logistic(θ_actual.ode.τ * 0.1) + 3.0)
...
function loss_function(θ_actual, t_max, t_obs_data, C_obs_data, D_obs_data, prob_base, u_base, t_vac_data, V_acc_obs_data, t0_val, w_D, w_V, w_S, eps_loss, features_grid)    
    ...
    if is_bad
        return 1.0e4 + 0.1 * sum(abs2, θ_actual), 1.0e4, 1.0e4, 1.0e4, 1.0e4
    end
    ...
    loss_C = mean((log.(C_pred_smooth .+ eps_loss) .- log.(C_obs_slice .+ eps_loss)).^2)
    loss_D = mean((log.(D_pred_smooth .+ eps_loss) .- log.(D_obs_slice .+ eps_loss)).^2)
    ...
    if idx_vac_max > 0
        ...
        vac_indices = Zygote.ignore_derivatives() do
            [searchsortedfirst(t_obs_window, t) for t in t_vac_window]
        end
        
        V_acc_pred = pred[8, vac_indices] 
        V_acc_pred_smooth = sqrt.(V_acc_pred.^2 .+ 1e-7)
        
        loss_V = mean((log.(V_acc_pred_smooth .+ eps_loss) .- log.(V_acc_obs_window .+ eps_loss)).^2)        
    else
        loss_V = 0.0 
    end

    # --- TEMPORAL SMOOTHNESS PENALTY ---
    # Evaluate the network over the precomputed grid
    nn_outputs, _ = nn(features_grid, θ_actual.nn, st)
    
    # Calculate the squared difference between consecutive points in time
    diffs = nn_outputs[:, 2:end] .- nn_outputs[:, 1:end-1]
    loss_smooth = mean(abs2, diffs)
    # -----------------------------------

    regularization = 1e-6 * sum(abs2, θ_actual.nn)
    
    # Combine everything
    total_loss = loss_C + (w_D * loss_D) + (w_V * loss_V) + (w_S * loss_smooth) + regularization
    
    return total_loss, loss_C, loss_D, loss_V, loss_smooth
end
...
function train_model(θ_initial, t_obs_data, C_obs_data, D_obs_data, t_vac_data, V_acc_obs_data, features_grid, training_schedule, prob_base, u_base, t0_val, w_D, w_V, w_S, eps_loss)
    loss_hist  = Float64[]
    ...
    for (i, stage) in enumerate(training_schedule)
        ...
        opt_loss = (ps, _) -> begin
            l_tot, _, _, _, _ = loss_function(ps, t_max, t_obs_data, C_obs_data, D_obs_data, prob_base, u_base, t_vac_data, V_acc_obs_data, t0_val, w_D, w_V, w_S, eps_loss, features_grid)
            return l_tot
        end
        ...
        cb = (state, loss) -> begin
            ps = state.u
            l_tot, l_C, l_D, l_V, l_smooth = loss_function(ps, t_max, t_obs_data, C_obs_data, D_obs_data, prob_base, u_base, t_vac_data, V_acc_obs_data, t0_val, w_D, w_V, w_S, eps_loss, features_grid)
            push!(loss_hist, l_tot)

            val_η  = 0.01 * logistic(ps.ode.η)
            val_ω  = 0.01 * logistic(ps.ode.ω)
            τ_real = 6.0 * logistic(ps.ode.τ * 0.1) + 3.0
            push!(param_hist, [val_η, val_ω, τ_real])

            if rand() < 0.1
            @printf("[Win: %d | Epoch: %d] TOT: %.4e | C: %.4e | D: %.4e | V: %.4e | SM: %.4e | Delay: %.2f\n",
                       t_max, length(loss_hist), l_tot, l_C, l_D, l_V, l_smooth, τ_real)
            end
            return false
        end

        if iter_adam > 0
            println("   ▶️  Running Adam (with ClipNorm)...")
...
t_obs, C_obs, D_obs, t_vac, V_acc_obs, t_init_vac, tspan = load_data(STATE_NAME, DATA_PATH, VAC_PATH, TOTAL_POP, 260)

# Extract time limits for safe passing
t0_val = Float64(tspan[1])
T_val  = Float64(tspan[2])

# Initialize Lux Network
seed = 42
...
prob_dde = DDEProblem(f_dde_closed, u_0, hist_func, tspan, θ;
                      constant_lags=[6.0 * logistic(θ.ode.τ * 0.1) + 3.0])
...
const eps_loss = 1e-3
# START TRAINING
θ_optimal, loss_history, param_history = train_model(
    θ, t_obs, C_obs, D_obs, t_vac, V_acc_obs,
    features_grid_static, 
    training_schedule, prob_dde, u_0, t0_val, 
    w_D, w_V, w_S, eps_loss
  )
...
prob_final = remake(prob_dde, p=θ_optimal, tspan=tspan,
                    constant_lags=[softplus(θ_optimal.ode.τ) + 1.0])
...
val_τ_final = 6.0 * logistic(θ_optimal.ode.τ * 0.1) + 3.0
